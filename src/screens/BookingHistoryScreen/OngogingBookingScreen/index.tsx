import { getTicketsOngoing } from "@src/store/actions/ticketAction";
...
const OngogingBookingScreen = ({ navigation }: Props) => {
  const newTicketsState = useAppSelector(selectOngoingTickets);
  const userState = useAppSelector(selectUser);
  const dispatch = useAppDispatch();
  useEffect(() => {
    dispatch(getTicketsOngoing(userState.id));
  }, []);
